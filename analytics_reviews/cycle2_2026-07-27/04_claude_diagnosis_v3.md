# Claude Diagnosis & Prioritized Plan — Cycle 2 — v3

**Input:** `01_cursor_report.md` + `03_codex_critique_round1.md` (10 issues — corrected count, see `03b`) + `03c_codex_critique_round2.md` (verdict: NEEDS_REVISION, 4 partial + 3 new) + `03a`/`03b_accept_reject_log` (23/23 total ACCEPT across both rounds, 0 REJECT).
**Status:** DRAFT v3 — revised per Codex round-2 review, pending round-3 re-review. **No production prompt, generator, or workflow file has been modified.**

## Changes from v2 (round 2 fixes)
...
- **Change — now locked, not placeholder:**
  - **Population, date range locked per Codex round 3:** all Phong Thủy Short scripts with `status` in `{video_ready, uploaded}` in `output/shorts/Phong Thủy/registry.json`, with `published_at`/`publish_at` between **2026-07-18T00:00:00Z and 2026-07-27T00:00:00Z** inclusive (this cycle's full available window, per `01_cursor_report.md`'s wide-window traffic-source query range) — a fixed, reproducible snapshot taken from the registry as it exists at the time this audit is run, not an open-ended "as of review" window. This yields the same 8 videos identified in this cycle's batch; if the registry has grown by the time the audit is actually executed, the date bound (not "current registry state") is what defines the population.
  - **Stratification:** by generator (currently 6 generators represented in this batch: `element_luck`, `western_zodiac`, `iching`, `storytelling`, `zodiac`, `element_color`).
...
- **Success criteria:** audit completed against the full locked population (8 scripts) with both passes recorded; decision threshold applied and stated explicitly (triggered or not).
...
*Next: submit this v3 diagnosis to Codex CLI for round-3 review.*
